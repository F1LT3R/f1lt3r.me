...
const getPages = (updatePage) => new Promise((resolve, reject) => {
    const readDirs = [];    
    const pages = {};
    ...
    readDir(pagesDirSrc).then(({dir}) => {
        for (const pageDir of dir) {
			// updatePage to only process the post that was changed 
			if (updatePage && !updatePage.includes(pageDir)) {
				continue
			}
...
export default (updatePage) => new Promise((resolve,reject) => {
    copyFiles(updatePage)
        .then(loadTemplates)
        .then((templates) => {
            // console.log(templates)
