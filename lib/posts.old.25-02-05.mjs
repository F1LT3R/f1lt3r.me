...
import prettyDates from './core/pretty-dates.mjs'
...
const getPostDetails = ({ file, fileDataUTF8 }) =>
    new Promise((resolve, reject) => {
        let details
        let datetime
        let postdate
...
const writeHTML = (html, fileDir, filePath) =>
    new Promise((resolve, reject) => {
...
const copyPostFilesToDist = (post) =>
    new Promise((resolve, reject) => {
        const { distDir, file } = post
        ...
        copyPostDirPromise().then(resolve).catch(reject)
    })

const readTemplateFile = (file, group, zone) =>
    new Promise((resolve, reject) => {
        fs.readFile(file, 'utf8', function (err, fileDataUTF8) {
            if (err) return reject(err)
...
const loadTemplates = () =>
    new Promise((resolve, reject) => {
        const {
            build: { templates: templateGroups },
        } = settings
...
const embedPostInHTMLTemplate = (post, templates) =>
    new Promise(async (resolve, reject) => {
        const { attributes, file, html, postdate, datetime } = post
        ...
        const main_template = handlebars.compile(templates.post.main_template.fileDataUTF8)({
            post,
            postdate,
        })
    ...
    new Promise((resolve, reject) => {
        copyFiles(updatePost)
            ...
            .then((templates) => {
                ...
                getPosts(updatePost).then((results) => {
                    // console.log(results)
